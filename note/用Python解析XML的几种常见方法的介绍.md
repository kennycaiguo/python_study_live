...
这是一个让Python不同的库使用相同API的一个比较常用的办法，而从Python
3.3开始ElementTree模块会自动寻找可用的C库来加快速度，所以只需要import xml.etree.ElementTree就可以了。
在CODE上查看代码片派生到我的代码片
...
![201549105948952.png(509×377)](https://img.jbzj.com/file_images/article/201504/201549105948952.png?20153911031)

参考：https://docs.python.org/2/library/xml.etree.elementtree.html
**2、xml.dom.***

文件对象模型（Document Object Model，简称DOM），是W3C组织推荐的处理可扩展置标语言的标准编程接口。一个 DOM
的解析器在解析一个XML文档时，一次性读取整个文档，把文档中所有元素保存在内存中的一个树结构里，之后你可以利用DOM
提供的不同的函数来读取或修改文档的内容和结构，也可以把修改过的内容写入xml文件。python中用xml.dom.minidom来解析xml文件，例子如下：
在CODE上查看代码片派生到我的代码片
...
![201549110218145.png(471×264)](https://img.jbzj.com/file_images/article/201504/201549110218145.png?20153911234)

**4、libxml2和lxml解析xml**

libxml2是使用C语言开发的xml解析器，是一个基于MIT
License的免费开源软件，多种编程语言都有基于它的实现，python中的libxml2模块有点小不足的是：xpathEval()接口不支持类似模板的用法，但不影响使用，因libxml2采用C语言开发的，因此在使用API接口的方式上难免会有点不适应。
在CODE上查看代码片派生到我的代码片
...
![201549110301575.png(457×318)](https://img.jbzj.com/file_images/article/201504/201549110301575.png?20153911315)
...
![201549110332839.png(513×57)](https://img.jbzj.com/file_images/article/201504/201549110332839.png?20153911348)
